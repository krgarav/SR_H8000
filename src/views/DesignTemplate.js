...
const DesignTemplate = () => {
    ...
    useEffect(() => {
        const handleBeforeUnload = (event) => {
            const confirmationMessage = 'Are you sure you want to leave this page? All unsaved data will be lost.';
            event.returnValue = confirmationMessage; // Standard for most browsers
            return confirmationMessage; // Required for some browsers
        };

        const handleNavigation = (event) => {
            if (event.type === 'POP' && !window.confirm('Are you sure you want to leave this page? All unsaved data will be lost.')) {
    ...
    const handleEyeClick = (selectedField, index) => {
        ...
        if (selectedField?.fieldType === "idField") {
            ...
            const index = parameters.findIndex((item) => isEqual(item.Coordinate, formattedSelectedFile));
            console.log(index)
        ...
        <>
            ...
            >
              ...
              /> */}
                    ...
                    {((selectedFieldType === "questionField" ||
                        selectedFieldType === "formField") && fieldType === "custom") && (
                            <Row>
                                <label htmlFor="example-text-input" className="col-md-2 ">
                                    Custom Value :
                                </label>
